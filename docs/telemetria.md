...
| `management.otlp.metrics.export.*` | `application.properties` | Endpoint, token e intervalo de envio (10s), lidos de variáveis de ambiente |
| `management.metrics.tags.application` | `application.properties` | Marca toda métrica com `application="hemotrack-backend"` |

O export vem **desligado por padrão**. Sem as variáveis abaixo o backend sobe normalmente, só não envia nada.

## Como ligar

1. Pegue as credenciais com quem administra a conta do Grafana Cloud: no portal, stack → card **OpenTelemetry** → **Configure**. A tela mostra o endpoint e gera o header `Authorization: Basic ...` pronto.
2. No **mesmo terminal** onde vai rodar o backend:

   ```bash
   export GRAFANA_OTLP_ENABLED=true
   export GRAFANA_OTLP_ENDPOINT="https://otlp-gateway-prod-sa-east-1.grafana.net/otlp/v1/metrics"
   export GRAFANA_OTLP_TOKEN="Basic <token-em-base64>"
   ./mvnw spring-boot:run
   ```

   Windows (PowerShell): `$env:GRAFANA_OTLP_ENABLED="true"` e assim por diante.

> **Nunca commite o token.** Ele só existe nas variáveis de ambiente.
...
- **Variáveis de ambiente valem só no terminal onde foram exportadas**, e só são lidas quando o processo sobe. Exportou depois de subir, ou em outra aba? Reinicie o backend no terminal certo.
